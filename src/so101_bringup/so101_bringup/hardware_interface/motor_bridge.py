...
CALIB_FILE = (
    pathlib.Path(get_package_share_directory("so101_bringup"))
    / "config/so101_follower_calibration.yaml"
)
...
class MotorBridge(Node):
    def __init__(self):
        ...
        self.joint_states_pub = self.create_publisher(JointState, "hardware_interface/joint_states", 10)
        ...
        print("calib file path", calib_path)
    ...
    def _timer_cb(self):
        ...
        try:
            ...
            positions = {}
            if self._home_offsets:
                for n, raw in raw_positions.items():
                    home = self._home_offsets.get(n, 0)
                    sign = JOINT_SIGN.get(n, 1.0)  # +1 or -1

                    # raw steps -> radians, then apply sign
                    positions[n] = sign * (raw - home) * (2 * math.pi) / 4096.0
            else:
                # before we have home_offsets, just put 0.0
                for n in raw_positions.keys():
                    positions[n] = 0.0
        ...
        if (
            ...
            for name in JOINTS:
                if name in self._limits and name in self._home_offsets:
                    ...
                    # Choose target_raw differently for gripper vs other joints
                    if name == "gripper":
                        # Go to minimum position (closed)
                        target_raw = low
                    else:
                        # Go to mid-range for normal joints
                        target_raw = int((low + high) / 2)

                    # Convert target_raw to radians relative to home, with sign
                    sign = JOINT_SIGN.get(name, 1.0)
                    self.current_commands[name] = (
                        sign * (target_raw - self._home_offsets[name]) * (2 * math.pi) / 4096.0
                    )

            self._initial_move_done = True
            self.get_logger().info("Issued initial command to move joints to initial positions.")
        ...
        try:
            ...
            for n, rad in self.current_commands.items():
                ...
                # URDF angle (rad) -> hardware angle (rad)
                hw_rad = sign * rad

                # hardware angle -> raw steps
                raw = int(home + hw_rad * self._steps_per_rad)
