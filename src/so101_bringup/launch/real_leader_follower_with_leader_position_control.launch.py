...
def generate_launch_description() -> LaunchDescription:
    ...
    leader_robot_bringup_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(pkg_bringup, "launch", "real_leader_with_position_control.launch.py")),
    )
    
    # real follower robot bringup node (with trajectory control)
    follower_robot_bringup_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(pkg_bringup, "launch", "real_follower_with_trajectory_control.launch.py")),
    )
