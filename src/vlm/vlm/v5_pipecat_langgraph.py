...
from services.stt.stt_qwen3_flash import Qwen3FlashSTTService
from services.tts.tts_qwen3_flash import QwenTTSService
...
def detect_objects(prompt: str) -> str:
    ...
    or compare what is currently visible — including questions about spatial
    relations (in front, behind, on, under, next to), colors, counts, or
    relative positions. Takes about 3-4 seconds.

    Args:
        prompt: A CLEAR, DETAILED question for the vision model.
                The user may speak in short, unclear, or incomplete sentences.
                You must REPHRASE their words into a full, specific question
                the vision model can answer well.

                Rules for building the prompt:
                  - Always frame it as a question.
                  - Make it self-contained (no pronouns like 'it', 'that').
                  - If the user asks about spatial relations, colors, or
                    counts, include those words explicitly.
                  - Keep it focused on what is currently visible.

                Examples:
                  User: "look at the table"
                    -> "What objects are currently on the table, and what
                        are their colors and positions?"
                  User: "is the dog behind the cat"
                    -> "Is the dog positioned behind the cat in the current
                        scene? Describe their relative positions."
                  User: "how many"
                    -> "How many distinct objects can you see in the scene
                        right now? List them with counts."
                  User: "what colors"
                    -> "What are the colors of every visible object in the
                        current scene?"
    """
