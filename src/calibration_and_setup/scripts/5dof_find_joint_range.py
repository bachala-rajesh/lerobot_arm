#!/usr/bin/env python3
"""Find range_min / range_max for each joint — 5DOF arm (6 servos).
...
Usage:
    python3 5dof_find_joint_range.py --arm follower
"""
import argparse
import datetime
import os
...
filename = f'5dof_range_{args.arm}_{now.strftime("%Y-%m-%d_%H-%M-%S")}.yaml'
filepath = os.path.join(backup_dir, filename)

with open(filepath, 'w') as f:
    f.write('# Joint range backup (5DOF)\n')
